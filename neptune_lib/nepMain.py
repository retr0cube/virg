...
class Init:
    def __init__(self, name):
        ...
        try:
            ...
            print(f"""\033[1;31;40m /!\ Error #001 : Unable to Overwrite Files (Try to Delete Them Manually)\033[0m
            """)

    def Dependencies(self, res, beh):

        if os.getcwd() == str(self.name):
            os.chdir(str(self.name))

        if res == True and beh == True:
            logging.debug('Behavior & Ressource set to TRUE')
            folders = ['{}_res'.format(self.name), '{}_beh'.format(self.name)]
            for folder in folders:
                os.mkdir(os.path.join(root_path, folder))
        elif res == True and beh == False:
            logging.debug('Ressource set to TRUE')
            os.mkdir('{}_res'.format(self.name))
        elif res == False and beh == True:
            logging.debug('Behavior set to TRUE')
